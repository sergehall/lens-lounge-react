...
export const getUserBlogsByCategory = (username: string): Record<string, BlogPreview[]> => {
    ...
    return Object.entries(allBlogMocks).reduce((acc, [category, blogsInCategory]) => {
        const userBlogs = blogsInCategory.filter(blog => blog.author.toLowerCase() === username.toLowerCase());
